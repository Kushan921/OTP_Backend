...
// // Extract OTP from body or page text
...
// // Extract link from body (Netflix or others)
// function extractLinkFromBody(body, keyword = "") {
//   const regex = new RegExp(
//     `https?:\\/\\/(www\\.)?[^\\s"]*${keyword}[^\\s">]*`,
//     "gi"
//   );
...
//       expiry_date: account.tokenExpires
//         ? new Date(account.tokenExpires).getTime()
//         : null,
//     });
...
//       new Date(account.tokenExpires).getTime() <= now
//     ) {
...
//       // No emails found, but keep trying for up to 2 minutes
//       const requestAge = Date.now() - new Date(otpRequest.requestedAt).getTime();
//       if (requestAge < 2 * 60 * 1000) { // 2 minutes
//         setTimeout(() => processOTPRequest(requestId), 10000); // Retry in 10 seconds
//         return;
//       } else {
//         await otpRequest.update({
//           status: 'failed',
//           error: 'No emails found within timeout period'
//         });
//         return;
//       }
//     }

//     // Process emails to find unprocessed OTP
...
//         // Check if this email matches the requested OTP type
//         const matchesType =
//           otpConfig.subjectKeywords.some(keyword =>
//             subject.toLowerCase().includes(keyword.toLowerCase())
//           ) ||
//           otpConfig.bodyKeywords.some(keyword =>
...
//         // Try extracting OTP from body
//         let otp = extractOTP(emailBody);
//         let otpSource = "email_body";

//         // Handle links (like Netflix household)
...
//     // No matching OTP found, retry if within timeout
//     const requestAge = Date.now() - new Date(otpRequest.requestedAt).getTime();
//     if (requestAge < 2 * 60 * 1000) { // 2 minutes
//       setTimeout(() => processOTPRequest(requestId), 15000); // Retry in 15 seconds
//       return;
//     } else {
//       await otpRequest.update({
//         status: 'failed',
//         error: 'No matching OTP found within timeout period'
//       });
//     }
...
//   if (!password) {
//     return res.status(400).send("Password is required in query params");
//   }
...
//   const redirectWithPassword = `${authUrl}&state=${encodeURIComponent(password)}`;
//   res.redirect(redirectWithPassword);
// });


// router.get("/callback", async (req, res) => {
//   const { code, state: password } = req.query;
//   if (!code || !password) {
//     return res.status(400).send("Missing code or password");
//   }
...
//   }
// });
// // Request OTP - creates a queued request
...
//         error: "accountType, otpType, and sessionId are required in request body"
//       });
//     }

//     if (!ACCOUNT_TYPE_CONFIGS[accountType]) {
//       return res.status(400).json({
//         error: `Unsupported account type: ${accountType}`
//       });
//     }

//     if (!ACCOUNT_TYPE_CONFIGS[accountType].otpTypes[otpType]) {
//       return res.status(400).json({
//         error: `Unsupported OTP type: ${otpType} for account type: ${accountType}`
//       });
//     }
...
//         error: "Session already has a pending OTP request",
...
//     processOTPRequest(otpRequest.id);
...
//       ...(otpRequest.status === 'completed' && {
//         otp: otpRequest.otp,
//         completedAt: otpRequest.completedAt
//       }),
...
// // Main route: fetch latest email and extract OTP
...
//       return res.status(400).json({ 
//         error: "accountType and otpType query parameters are required" 
//       });
//     }
...
//     // Fetch emails from the last 10 minutes for this account type
//     const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000);
//     const query = `from:${config.sender} after:${Math.floor(tenMinutesAgo.getTime() / 1000)}`;

//     const messagesRes = await gmail.users.messages.list({
//       userId: "me",
//       maxResults: 20, // Fetch more emails to handle multiple OTPs
//       q: query,
//     });

//     if (!messagesRes.data.messages || messagesRes.data.messages.length === 0) {
//       return res.status(404).json({ 
//         message: `No recent emails found from ${config.sender}` 
//       });
//     }

//     // Process emails to find unprocessed OTP of the requested type
//     for (const message of messagesRes.data.messages) {
//       // Check if this message has already been processed
//       const alreadyProcessed = await ProcessedEmail.findOne({
//         where: { messageId: message.id }
...
//         if (!emailBody) {
//           continue; // Skip emails without readable body
//         }
...
//         if (!matchesType) {
//           continue; // Not the right OTP type
//         }
...
//         if (otp !== "No OTP found") {
//           // Mark this email as processed to prevent concurrent access
//           await ProcessedEmail.create({
//             messageId: message.id,
//             emailAccountId: parseInt(emailAccountId),
//             accountType,
...
//       } catch (emailError) {
//         console.error(`Error processing message ${message.id}:`, emailError);
//         continue; // Skip this email and try the next one
//       }
//     }

//     // No matching unprocessed OTP found
//     res.status(404).json({ 
//       message: `No unprocessed ${otpType} OTP found for ${accountType} in recent emails` 
//     });
...
router.get("/connect", (req, res) => {
  const { password } = req.query;
  if (!password) return res.status(400).send("Password is required in query params");
  ...
  res.redirect(`${authUrl}&state=${encodeURIComponent(password)}`);
});
